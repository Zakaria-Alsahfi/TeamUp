...
//  SeniorProject
//
//  Created by zakaria alsahfi on 10/14/18.
...
//        self.navigationItem.backBarButtonItem?.tintColor = .white
    ...
    func getUserLocation(){
        ...
        currentUser.fetchInBackground(block: { (User, error) in
            if let user = User as? PFUser {
                let value = user.object(forKey: "distance") as? Int
                self.sliderValue = Float(value!)
                print(self.sliderValue)
                self.slider.setValue(self.sliderValue, animated: true)
                if Int(self.sliderValue) == 1 {
                    self.distanceLabel.text = String(Int(self.sliderValue)) + " mile"
                }else {
                    self.distanceLabel.text = String(Int(self.sliderValue)) + " miles"
                }
                let location = user.object(forKey: "location") as! PFGeoPoint
                self.latitude = location.latitude
                self.longitude = location.longitude
                self.coords = CLLocationCoordinate2D(latitude: self.latitude, longitude: self.longitude)
                self.displayLocation(Location: self.coords!)
                print(self.latitude)
                print(self.longitude)
            }
        })
    ...
    func setUpView(){
        ...
        slider.topAnchor.constraint(equalTo: textLabel.bottomAnchor, constant: 20).isActive = true
        ...
        saveButton.topAnchor.constraint(equalTo: myView.bottomAnchor, constant: 10).isActive = true
